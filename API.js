...
app.post('/getConfig', async (req, res, _) => {
                ...
                if (result != undefined && result.length != 0) {
                    ...
                    fs.writeFile(filename, bytes, "binary", function (err) {
                        if (err) {
                            ...
                            const type1 = func.getFileType(filename);
                             console.log(type1);
                            const sql = `INSERT into downloadrecords VALUES('${token}','${filename}',unix_timestamp(now()),'${type1}');`
                            exec(sql).then(async (result) => {
                                console.log(result)
                                if (result == undefined || result.length == 0) {
                                    res.json({
